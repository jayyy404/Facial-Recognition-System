...
async function loadState() {
  ...
  const { users, logs, attendance } = await fetch('/api/get-state').then((res) =>
    res.json()
  );
...
function updateDashboard() {
   ...
   */
  function splitIntoRecognizedUnrecognized() {
  ...
  const todayAttendance = data.attendance.filter(l => {
    const d = new Date(l.date);
    const d2 = new Date();

    console.log(d, d2, d.getDate() - d2.getDate());
    return d.getDate() === d2.getDate();
  }).reduce((p, c) => {
    if (p.find(user => user.user_id === c.user_id)) return p;
    return [...p, c];
  }, []);
  ...
  const todayAttendanceRate = Math.round(
    (todayAttendance.length / (data.users.length || 1)) * 100
  );
  ...
  /** Update chart */
  updateChart(todayAttendanceRate);
